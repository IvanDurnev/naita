...
REFERENCE_PROFILE = '''{
    "main_vacancy": желаемая вакансия,
    "education": {
        "main_education": {
    ...
    "work_experience_start": год начала работы,
    "work_experience": [
        {
            "company": название места работы or "",
            "dates": годы начала и окончания работы на этом месте or "",
            "responsibilities": основные обязанности и задачи на этом рабочем месте,
            "achievements": какие достижения были на этом рабочем месте?
        }
    ],
    "hard_skills": какими профессиональными компетенциями обладаешь для выбранной вакансии? or "",
    "soft_skills": какими личными качествами или навыками обладаешь для выбранной вакансии? or "",
    "achievements": какие имеешь профессиональные награды или премии, какие есть достижения? or "",
    "additional_information": дополнительная информация, которую кандидат хотел бы указать
}'''
...
YA_GPT_V2T = '''Посмотри текст, если в нем есть обращение к кому-либо на "вы", то замени его на обращение на "ты" во всех случаях и формах.
Если обращений нет, то ничего менять не нужно и нужно вернуть исходный текст.

Если обращения есть, то поменяй и верни исправленный текст.'''

WAIT_FOR_CV = 'Готовлю твоё резюме, пришлю сюда, подожди, пожалуйста.'
...
Где должности или другие термины указаны просторечно - переделай в правильные официальные названия.
...
def get_cv_fields(user):
    ya_gpt_client = YAGPT()
    prompt = f'''Посмотри информацию обо мне:
Меня зовут: {user.first_name} {user.last_name}
Телефон: {user.phone}
Email: {user.email}
Ссылка на фото: {user.get_avatar_external()}
Краткий профиль: {user.profile}
История сообщений с ассистентом: {user.get_ya_thread_messages()}

Составь для меня хорошее подробное CV.
Где не хватает информации - распиши красиво и подробно имеющуюся.
Где должности или другие термины указаны просторечно - переделай в правильные официальные названия.

Обязательные информационные секции:


Верни моё CV строго в формате JSON:
{{
name: Имя,
profession: Профессия (специальность),
description: Краткое описание меня как соискателя,
email: email,
photo: cсылка на фото,
city: мой город,
phone: телефон,
job_experience: [{{'name': название компании, 'position': string должность, 'years': годы работы, 'responsibilities': [основные обязанности и задачи], 'achievements': [основные достижения]}}],
education: [{{'name': название образовательного учреждения, profession: специальность строкой (не списком),  'years': годы обучения}}],
hard_skills: [мои Hard skills] or [],
soft_skills: [мои Soft skills] or [],
other: [другая информация обо мне] or []
}}
'''
    response = ya_gpt_client.completion(prompt).replace("```", "").strip()
    return response
